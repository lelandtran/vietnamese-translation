...
import { db } from '../../lib/db';
import openai from '../../lib/openai';
import { words, normalizedWords, wordNormalization, meanings } from '../../lib/schema';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const normalizedWordResult = await db.select()
    ...
    if (normalizedWordResult.length > 0) {
      ...
      const wordLinks = await db.select({
      ...
      for (const wordId of wordIds) {
        ...
        const meaningResults = await db.select()
          .from(meanings)
...
async function storeInDatabase(normalizedWord: string, wordsData: WordWithMeanings[]) {
  try {
    // Insert the normalized word
    const [normalizedResult] = await db.insert(normalizedWords)
    ...
    for (const wordData of wordsData) {
      // Insert the word
      const [wordResult] = await db.insert(words)
      ...
      // Create the normalization link
      await db.insert(wordNormalization)
        .values({ wordId, normalizedId });
      
      // Insert meanings
      for (const meaning of wordData.meanings) {
        await db.insert(meanings)
          .values({
            wordId,
